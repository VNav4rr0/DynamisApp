...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, ScrollView, ActivityIndicator, Keyboard, Alert } from 'react-native';
...
import { MainTabParamList, AppStackParamList } from '../../App';
type GerenciarInformacoesScreenProps = NativeStackScreenProps<AppStackParamList, 'GerenciarInformacoes'>;

const GerenciarInformacoesScreen: React.FC<GerenciarInformacoesScreenProps> = () => {
    ...
    const validateInputs = useCallback(() => {
        Keyboard.dismiss();
        let isValid = true;
        setNomeError(false); setIdadeError(false); setAlturaError(false); setGeneroError(false);
        setPesoAtualError(false); setPesoMetaError(false); setObjetivoError(false); setNivelAtividadeError(false);
        setEmailError(false); setSenhaError(false); setCurrentPasswordError(false);
        ...
        if (showPasswordFields) {
            if (!currentPassword.trim()) { setCurrentPasswordError(true); isValid = false; showAlert(t('manageInfo.requiredFieldsTitle'), t('manageInfo.enterCurrentPasswordMessage'), 'error'); }
            if (!senha.trim()) { setSenhaError(true); isValid = false; showAlert(t('manageInfo.requiredFieldsTitle'), t('manageInfo.enterNewPasswordMessage'), 'error'); }
            else if (senha.length < 6) { setSenhaError(true); isValid = false; showAlert(t('manageInfo.weakPasswordTitle'), t('manageInfo.weakPasswordMessage'), 'error'); }
        } else {
            const isEmailChanged = email !== (auth.currentUser?.email || '');
            if (isEmailChanged && !currentPassword.trim()) { setCurrentPasswordError(true); isValid = false; showAlert(t('manageInfo.confirmationNeededTitle'), t('manageInfo.emailChangeRequiresCurrentPasswordMessage'), 'error'); }
        }
        return isValid;
    ...
    // --- FUNÇÃO DE CÁLCULO DE CALORIAS E MACROS ---
    const calculateNutritionalGoals = useCallback((
        peso: number,
        altura: number,
        idade: number,
        genero: 'Homem' | 'Mulher',
        ...
        const atividadeMultiplicador: Record<string, number> = {
            'leve': 1.375,
            'moderado': 1.55,
            'intenso': 1.725,
        };
        const TMB = genero === 'Homem' 
            ? (10 * peso) + (6.25 * altura) - (5 * idade) + 5
            : (10 * peso) + (6.25 * altura) - (5 * idade) - 161;
        const metaCalorias = TMB * (atividadeMultiplicador[nivelAtividade] || 1);

        const macros: { proteina: number; carboidratos: number; gordura: number; } = {
            proteina: 0,
            carboidratos: 0,
            gordura: 0,
        };
        if (objetivo === 'emagrecer') {
            macros.proteina = peso * 2;
            macros.carboidratos = (metaCalorias * 0.40) / 4;
            macros.gordura = (metaCalorias * 0.30) / 9;
        } else if (objetivo === 'ganhar_massa') {
            macros.proteina = peso * 2.2;
            macros.carboidratos = (metaCalorias * 0.50) / 4;
            macros.gordura = (metaCalorias * 0.25) / 9;
        } else if (objetivo === 'manter_peso') {
            macros.proteina = peso * 1.8;
            macros.carboidratos = (metaCalorias * 0.45) / 4;
            macros.gordura = (metaCalorias * 0.30) / 9;
        } else if (objetivo === 'definicao') {
            macros.proteina = peso * 2.5;
            macros.carboidratos = (metaCalorias * 0.30) / 4;
            macros.gordura = (metaCalorias * 0.45) / 9;
        }
        return { metaCalorias: Math.round(metaCalorias), macros: {
            proteina: Math.round(macros.proteina),
            carboidratos: Math.round(macros.carboidratos),
    ...
    const handleSaveChanges = useCallback(async () => {
        ...
        try {
            ...
            // NOVO: Calcular as novas metas nutricionais
            const novosCalculos = calculateNutritionalGoals(
                Number(pesoAtual),
                Number(altura),
                Number(idade),
                genero!,
            ...
            await updateDoc(userDocRef, {
                ...
                email: email.trim(),
                // NOVO: Salvar os novos cálculos no banco
                calculosNutricionais: novosCalculos,
            });
    ...
    }, [nome, idade, altura, genero, pesoAtual, pesoMeta, objetivo, nivelAtividade, email, senha, currentPassword, showPasswordFields, showAlert, validateInputs, t, calculateNutritionalGoals]);
