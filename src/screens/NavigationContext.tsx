...
const NutricionistaAccessScreen: React.FC<NutricionistaAccessScreenProps> = ({ navigation }) => {
    ...
    const handleCodeChange = useCallback((text: string, index: number) => {
        ...
        else if (newText.length === 0 && index > 0) {
            codeInputRefs.current[index - 1]?.focus();
        }
    }, [accessCodeParts]);
    ...
    const handleAccessCodeVerify = async () => {
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                setTimeout(() => {
                    // CORREÇÃO CRUCIAL: USAR navigation.replace PARA NAVEGAR PARA O AppStack
                    // Isso substitui a pilha de navegação atual (AuthStack) pela tela Nutricionista (no AppStack).
                    // navigation.replace('NomeDaRotaNoAppStack', { params... })
            ...
            if (error.code === 'permission-denied') {
                showAlert('Erro de Permissão', 'Acesso negado. Verifique as regras do Firestore.');
            } else {
